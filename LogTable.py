...
def connect_azure_table():
    ...
    with TableServiceClient(endpoint=os.getenv('AZURE_LOG_TABLE_URI')) as table_service_client:
        properties = table_service_client.get_table_client(os.getenv('AZURE_LOG_TABLE_NAME'))
        return properties
...
def create_entity(Partition, User, FromLang, ToLang, FromText, ToText, FromDoc, ToDoc):
    ...
    table_service_client = TableServiceClient(endpoint=os.getenv('AZURE_LOG_TABLE_URI'))
    table_client = table_service_client.get_table_client(table_name=os.getenv('AZURE_LOG_TABLE_NAME'))
